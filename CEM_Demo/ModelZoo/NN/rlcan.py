...
class LocalCALayer(nn.Module):
    def __init__(self, channel, reduction=16):
        super(LocalCALayer, self).__init__()
        self.conv_du = nn.Sequential(
            nn.Conv2d(channel, channel // reduction, kernel_size=3, stride=2, padding=1, bias=True),
            nn.ReLU(inplace=True),
            nn.Conv2d(channel // reduction, channel, kernel_size=3, stride=2, padding=1, bias=False),
...
class RCAB(nn.Module):
    def __init__(
        ...
        modules_body.append(LocalCALayer(n_feat, reduction))
