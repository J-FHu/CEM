...
class Net(nn.Module):  # 16,16,8,8
    def __init__(self, num_channels=3, growthRate=16, nDenselayer=8, nBlock=8):
        super(Net, self).__init__()

        self.conv1 = nn.Conv2d(num_channels, growthRate, kernel_size=3, padding=1, bias=True)
        ...
        self.convt2 = nn.ConvTranspose2d(in_channels=128, out_channels=128, kernel_size=4, stride=2, padding=1,
                                         bias=True)

        self.conv2 = nn.Conv2d(in_channels=128, out_channels=num_channels, kernel_size=3, padding=1, bias=True)
