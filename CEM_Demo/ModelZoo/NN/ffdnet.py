...
class FFDNet(nn.Module):
    def __init__(self, in_nc=3, out_nc=3, nc=96, nb=12, sigma=50, act_mode='R'):
        """
        # ------------------------------------
    ...
    def forward(self, x):
        # s = torch.from_numpy(np.ascontiguousarray(x.cpu())).permute(2, 0, 1).float().unsqueeze(0)
        sigma = torch.full((1,1,1,1), self.sigma/255).cuda()
        h, w = x.size()[-2:]
